...
if (isset($_POST['submit'])) {
    ...
    $sql = "INSERT INTO tb_barang (nama_barang, stok, satuan, tgl_kadaluarsa, nama_gambar, harga) VALUES ('$nama', '$stok', '$satuan', '$tgl_exp', '$nama_file_db', '$harga')";
    if (mysqli_query($koneksi, $sql)) { $status_proses = "success"; } else { $status_proses = "error"; $error_msg = mysqli_error($koneksi); }
}
?>
...
<head>
    ...
    <style>
        :root { --primary: #4a90e2; --bg-body: #f4f7fe; --success: #22c55e; }
        ...
        label { display: block; margin-bottom: 5px; font-weight: 500; color: #64748b; font-size: 12px; }
        .form-control { width: 100%; padding: 10px; border: 1px solid #e2e8f0; border-radius: 8px; box-sizing: border-box; }
        .btn-save { background: var(--success); color: white; border: none; padding: 10px; width: 100%; border-radius: 8px; font-weight: 600; cursor: pointer; }
        .btn-back { display: block; text-align: center; margin-top: 15px; color: #718096; text-decoration: none; font-size: 13px; }
        #preview { width: 80px; height: 80px; object-fit: cover; border-radius: 8px; margin-top: 10px; display: none; border: 1px solid #e2e8f0; }
    </style>
</head>
<body>
    ...
    <main class="main">
        <header class="top-header">
            <h3 style="margin:0; font-size: 18px; color: #334155;"><i class="fa-solid fa-boxes-stacked" style="color: var(--primary); margin-right: 10px;"></i> Tambah Barang</h3>
        ...
        <div class="form-card">
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <a href="lihat.php" class="btn-back">Kembali</a>
            </form>
        </div>
    </main>
    ...
    <script>
        ...
        <?php if($status_proses === "success"): ?>
            Swal.fire('Berhasil!', 'Data tersimpan', 'success').then(() => { window.location='lihat.php'; });
        <?php elseif($status_proses === "error"): ?>
            Swal.fire('Gagal!', 'Error: <?= addslashes($error_msg); ?>', 'error');
        <?php endif; ?>
    </script>
</body>
</html>